...
const deepMessages = [
  ...
  `<p>Aku tahu dadamu terasa sangat sesak malam ini karena tumpukan overthinking yang tak berujung, bub. Mari kita urai benang kusut itu bersama-sama.</p>
               <p class="border-l-2 border-pink-400 pl-3 italic text-pink-300 bg-white/5 py-1.5 rounded-r-xl">"Bicarakan padaku tanpa ragu. Setiap keluh kesahmu adalah simfoni penting yang ingin selalu kudengarkan."</p>`,
  ...
  `<p>Setiap kali kepalamu mulai terasa berat oleh kebisingan cemas, bub, pejamkan matamu sejenak dan bayangkan betapa damainya pelukan kita.</p>
  ...
  `<p>Setiap kali kecemasan itu datang menyerang, bub, tarik napasmu sedalam mungkin dan rasakan betapa udara semesta ini sangat mendukung kehidupanmu.</p>
               <p class="border-l-2 border-pink-400 pl-3 italic text-pink-300 bg-white/5 py-1.5 rounded-r-xl">"Kamu aman, kamu terlindungi, dan kamu memiliki cinta tak terbatas dari diriku yang selalu menjagamu."</p>`,
  ...
  `<p>Sayang kesayanganku, tolong sisakan sedikit dari cinta luasmu itu untuk mendekap dan menyayangi dirimu sendiri hari ini.</p>
               <p>Kamu sudah terlalu banyak peduli pada orang lain, bub, kini saatnya fokus memulihkan kedamaian di batinmu yang lelah.</p>`,
  ...
  `<p>Setiap malam yang melelahkan ini akan segera berganti dengan pagi yang cerah, bub, membawa sejuta kehangatan baru bagi hatimu.</p>
               <p class="border-l-2 border-pink-400 pl-3 italic text-pink-300 bg-white/5 py-1.5 rounded-r-xl">"Istirahatlah kesayanganku, biarkan cintaku yang bekerja menjaga kedamaian batinmu sepanjang malam ini."</p>`,
  ...
  `<p>Kamu adalah rasi bintangmu yang paling memikat, bub, cahaya indah yang takkan pernah pudar oleh berjalannya sang waktu.</p>
               <p>Tolong tetaplah hidup, tetaplah bertumbuh dengan keanggunanmu, dan mari kita menangkan pertarungan takdir ini bersama.</p>`,
...
// ========================================================
// INTEGRASI PROGRESIF WEB APP (PWA ENGINE ALAR YT STYLE)
// ========================================================
let deferredPrompt;
const pwaInstallBtn = document.getElementById("pwaInstallBtn");
const pwaGuideModal = document.getElementById("pwaGuideModal");
const closePwaGuide = document.getElementById("closePwaGuide");
const understandPwaBtn = document.getElementById("understandPwaBtn");

// Deteksi cara aplikasi dijalankan (Melalui PWA standalone atau Browser biasa)
const isRunningStandalone = () => {
  return window.matchMedia("(display-mode: standalone)").matches || window.navigator.standalone === true;
};

// Deteksi Event Instalasi dari Browser
window.addEventListener("beforeinstallprompt", (e) => {
  // Mencegah mini-infobar default muncul pada perangkat mobile
  e.preventDefault();
  // Simpan event sehingga bisa dipicu nanti secara kustom
  deferredPrompt = e;

  // Hanya tampilkan tombol "Instal App" jika aplikasi belum dijalankan dalam mode standalone
  if (pwaInstallBtn && !isRunningStandalone()) {
...
if (pwaInstallBtn) {
  pwaInstallBtn.addEventListener("click", async () => {
    // Jika browser mendukung instalasi otomatis kustom (Android Chrome, Desktop, Edge)
    if (deferredPrompt) {
      // Tampilkan prompt instalasi bawaan OS/Browser
      deferredPrompt.prompt();
      // Tunggu pilihan pengguna (Accept / Dismiss)
      const { outcome } = await deferredPrompt.userChoice;
      if (outcome === "accepted") {
        console.log("Pengguna menyetujui instalasi aplikasi Memory Vault.");
      }
      // Bersihkan objek prompt karena hanya bisa digunakan sekali
      deferredPrompt = null;
      // Sembunyikan tombol setelah aksi instalasi dipicu
      pwaInstallBtn.style.display = "none";
    } else {
      // Jika browser tidak mendukung install kustom otomatis (e.g. iOS Safari / Firefox mobile)
      // Tampilkan modal petunjuk kustom interaktif (ala instalasi manual YouTube di iOS)
...
// Handler penutupan panduan PWA
if (closePwaGuide) {
  closePwaGuide.onclick = () => {
    pwaGuideModal.classList.remove("active");
    document.body.style.overflow = "";
  };
}
if (understandPwaBtn) {
  understandPwaBtn.onclick = () => {
    pwaGuideModal.classList.remove("active");
    document.body.style.overflow = "";
  };
}

// Deteksi ketika aplikasi sukses diinstal oleh pengguna
window.addEventListener("appinstalled", (evt) => {
  console.log("Memory Vault - Arina sukses diinstal di perangkat!");
  if (pwaInstallBtn) pwaInstallBtn.style.display = "none";
  triggerMassiveCelebration(); // Berikan sambutan kembang api romantis sebagai selebrasi instalasi
});

// Sembunyikan tombol PWA secara dinamis jika dibuka langsung di aplikasi (Standalone Mode)
if (isRunningStandalone() && pwaInstallBtn) {
  pwaInstallBtn.style.display = "none";
}

// Registrasi Service Worker Dinamis Menggunakan Blob Data
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    const swContent = `
            const CACHE_NAME = 'arina-vault-cache-v1';
            const ASSETS = [
              '/',
    ...
    navigator.serviceWorker
      ...
      .catch((err) => {
        console.log("Pengecualian PWA Service Worker gagal didaftarkan:", err);
      });
  });
}
// ==========================================
...
window.addEventListener("load", () => {
  ...
  // Deteksi pertama kali pengguna membuka link di luar PWA untuk menawarkan instalasi kustom
  setTimeout(() => {
    if (!isRunningStandalone() && !localStorage.getItem("pwaPromptShown")) {
      // Tampilkan tombol pasang secara dinamis bagi browser pendukung
      if (pwaInstallBtn) {
        pwaInstallBtn.style.display = "flex";
      }
