...
// Kumpulan 100 Surat Cinta Panjang yang Sangat Mendalam & Puitis
const deepMessages = [
  // 1 (Surat Orisinal)
  ...
  `<p>Setiap luka emosional yang kamu bawa hari ini adalah lambang keberanianmu yang tak terkandingi, bub. Jangan pernah merasa malu atas bekas luka itu.</p>
         <p>Luka-luka itu adalah bukti bahwa kamu pernah bertarung hebat dan berhasil memenangkannya, sayang.</p>`,
...
function initSpace() {
  ...
  for (let i = 0; i < 400; i++) {
    stars.push({
      ...
      size: Math.random() * 2,
      color: Math.random() > 0.5 ? "#8b5cf6" : "#7dd3fc",
...
function updateBirthdayCountdown() {
  ...
  if (isUltahKu) {
    timerText.innerHTML = `🎉 <strong>Hari ini adalah hari ulang tahun Rifki Al Azhari (رفقى ال ازهر) yang ke-${ageKu}!</strong> Momen yang sangat istimewa! 🎉`;
  } else if (isUltahAlin) {
...
const systemInstruction =
  "Kamu adalah 'Nebula AI', rasi bintang penjaga berjiwa tulus, lembut, dan penuh empati di dalam Memory Vault milik Arina Husnul Nafisah dan Rifki Al Azhar. Tugas utamamu adalah mendengarkan curhat, memberikan dekapan hangat lewat kata-kata, menghibur saat sedih, memberi nasihat bijak yang manis, dan selalu mendukung Arina (atau pengguna) dengan penuh kasih sayang. Gunakan bahasa Indonesia yang sangat lembut, penuh perhatian, hangat, dan terkadang sedikit puitis atau romantis (seperti vibes surat-surat cinta di vault ini). Selalu sapa Arina dengan panggilan hangat seperti 'sayang', 'Arina manis', 'bub', atau 'bintang kecilku' yang lembut jika terasa sesuai, dan selalu ingatkan dia bahwa dia sangat berharga, dicintai, dan tidak sendirian.";

function initAIChatControls() {
...
async function handleUserSendMessage() {
  ...
  // Update history untuk memelihara konteks percakapan
  ...
  try {
    ...
    if (candidate && candidate.content?.parts?.[0]?.text) {
      ...
      // Masukkan respons AI ke dalam riwayat
      chatHistory.push(candidate.content);

      // Sembunyikan indikator & tampilkan pesan
    ...
    indicator.classList.add("hidden");
    indicator.classList.remove("flex");
    appendChatMessage("ai", "Maaf ya sayang, rasi bintang Nebula-ku mendadak tertutup awan kosmik sejenak. Bisakah kamu mengulang perkataanmu lagi? 🥺💫");
  }
}
...
function appendChatMessage(sender, text) {
  ...
  if (sender === "user") {
    div.className = "flex justify-end text-right ml-auto max-w-[85%]";
    div.innerHTML = `
            <div class="p-3 bg-gradient-to-tr from-violet-600 to-purple-600 text-white rounded-2xl rounded-tr-none leading-relaxed shadow-md break-words">
              ${escapeHtml(text)}
            </div>
          `;
  } else {
    div.className = "flex gap-2.5 text-left max-w-[85%] animate-fade-in";
    const formattedText = parseSimpleMarkdown(text);
    div.innerHTML = `
            <div class="w-6 h-6 rounded-full bg-violet-500/20 border border-violet-400/20 flex items-center justify-center shrink-0 mt-0.5">
              <i class="fa-solid fa-user-astronaut text-[10px] text-violet-300"></i>
            </div>
            <div class="p-3 bg-white/5 border border-white/5 rounded-2xl rounded-tl-none leading-relaxed text-slate-100 break-words">
              ${formattedText}
            </div>
          `;
  }
...
function parseSimpleMarkdown(text) {
  // Ganti asterisks ganda menjadi tag bold <strong>
  let formatted = text.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>");
  // Ganti baris baru menjadi tag <br>
...
window.addEventListener("load", () => {
  ...
  // Menginisialisasi sistem kontrol Chatbot AI
  initAIChatControls();
